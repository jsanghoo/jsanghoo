...
def naiveSoftmaxLossAndGradient(
    ...
    cond_prob = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(cond_prob[outsideWordIdx])

    # Adjust the probability for the target word
    cond_prob[outsideWordIdx] -= 1.0

    # Compute the gradients
    gradCenterVec = np.dot(outsideVectors.T, cond_prob)
    gradOutsideVecs = np.outer(cond_prob, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    # Compute the loss and gradients for the positive sample (outside word)
    ...
    # Compute the loss and gradients for the negative samples
    for idx in negSampleWordIndices:
        u_k = outsideVectors[idx]
        z = sigmoid(np.dot(u_k, centerWordVec))
        loss += -np.log(1.0 - z)
        gradCenterVec += z * u_k
        gradOutsideVecs[idx] += z * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        c_loss, c_gradCenterVec, c_gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec, outsideWordIdx, outsideVectors, dataset
        )
        
        loss += c_loss
        gradCenterVecs[centerWordIdx] += c_gradCenterVec
        gradOutsideVectors += c_gradOutsideVecs
